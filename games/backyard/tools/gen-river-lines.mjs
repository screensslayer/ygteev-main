// Record River's library cutscene lines.
//
//   SUPABASE_ANON=... node tools/gen-river-lines.mjs [--force] [--only 3,4]
//
// Lines are imported from src/glowlands/data/river-lines.js — the exact
...
//
// Writes public/voices/river-{1..10}.mp3 and bumps VOICE_V in the game
// source so no browser serves a stale take.
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { RIVER_LINES, RIVER_VOICE_ID } from '../src/glowlands/data/river-lines.js';
...
const todo = RIVER_LINES.filter(({ id }) => {
  if (only && !only.has(id)) return false;
  if (!force && fs.existsSync(path.join(OUT, `river-${id}.mp3`))) return false;
  return true;
});
if (!todo.length) {
...
for (let i = 0; i < todo.length; i += BATCH) {
  ...
  const res = await fetch(FN, {
    ...
    body: JSON.stringify({
      ...
      lines: batch.map(({ id, text }) => ({ name: `river-${id}`, text })),
    }),
  });
  ...
  for (const c of json.clips) {
    ...
    const line = batch.find((b) => `river-${b.id}` === c.name);
