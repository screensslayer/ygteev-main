...
| Live groupmates | Community garden only. Primary transport: the dedicated position relay at `wss://ygteev-relay.fly.dev` (`relay/` in this repo) — joins are verified against the player's Supabase JWT + `by_member_status`, and positions arrive as 10 Hz server snapshots, keeping traffic linear (and cost flat) as gardens fill. Automatic fallback: the private Supabase channel `by:garden:{group_id}` (membership-gated RLS on `realtime.messages`) whenever the relay is unreachable or refuses the join. Groupmates appear with their real outfit + name tag and animate as they walk |
