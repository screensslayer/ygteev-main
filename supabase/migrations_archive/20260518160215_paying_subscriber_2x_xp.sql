-- Paying-subscriber 2x XP multiplier.
-- `is_paying_subscriber` is the stricter sibling of `is_pro`: it does NOT
-- grant Pro to youth-group members, only to users with an active Apple
-- subscription. Used to apply a 2x multiplier on XP credited to profiles
-- (base values still recorded on day/step/milestone progress rows for audit).

create or replace function public.is_paying_subscriber(_user_id uuid)
returns boolean
...
grant execute on function public.is_paying_subscriber(uuid) to authenticated, service_role;

-- ----------------------------------------------------------------------------
-- complete_pastor_plan_day: same logic, plus xp multiplier
-- ----------------------------------------------------------------------------
create or replace function public.complete_pastor_plan_day(_plan_id uuid, _day_number integer, _answers jsonb default '[]'::jsonb)
returns jsonb
...
begin
  ...
  -- Paying Apple-subscribers earn 2x XP. Multiplier applies to the final
  -- credited balance only; per-row earnings (day/milestone/completion) stay
  -- recorded at base values for audit.
  _xp_base := _total_xp;
...
-- ----------------------------------------------------------------------------
-- complete_plan_step: same logic, plus xp multiplier on both update branches
-- ----------------------------------------------------------------------------
create or replace function public.complete_plan_step(_plan_id uuid, _day_id uuid, _step bible_plan_step, _answers jsonb default '{}'::jsonb)
returns jsonb
language plpgsql security definer
set search_path = public
as $function$
...
begin
  ...
  if _is_day_now_complete and not exists (
    ...
    -- Paying Apple-subscribers earn 2x XP. Multiplier applies to the final
    -- credited balance only; per-row earnings stay at base values for audit.
    _xp_base := _total_xp;
    if public.is_paying_subscriber(_uid) then
      _xp_multiplier := 2;
      _total_xp := _total_xp * _xp_multiplier;
    ...
    -- Step-only path (day not yet complete). Multiplier applies here too.
    _xp_base := _total_xp;
    if public.is_paying_subscriber(_uid) then
      _xp_multiplier := 2;
      _total_xp := _total_xp * _xp_multiplier;
