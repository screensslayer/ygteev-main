...
*Assembled edition — 2026-08-03. All numeric values are tuning placeholders unless marked LOCKED.*

---

## Executive summary

**What this is.** The Glowlands is the world expansion of the existing YGTeeV Backyard — the cozy low-poly three.js web game inside the YGTeeV teen Bible-reading app. It turns the Backyard from a destination into a doorstep: the player steps through a new garden gate into a ring of ten zones creeping with the Gloom, a darkness that feeds on lies, fear, and forgetting. Every system the player already runs — daily Bible-plan reading, water/XP, fruit-for-gold farming, Ember the dragon, the Garden League — becomes the power system of a journey to Everlight City, the source of all light.

**The three load-bearing ideas.**

1. **Truth & Light combat.** Gloomlings attack with written lies ("You're alone"); the player answers with the right Scripture from their Verse Satchel. Right truth = a Lightburst and the darkness flees; wrong = fear closes in. No HP, no death, no gore — darkness always flees light. Verses are earned only by library study and real Bible-plan reading. (Ch. 2)
2. **The Lantern.** Real daily reading in the app lights the player's lantern; brightness (Spark → Flame → Beacon → Radiant) gates frontier access — dark zones and dark missions — and never anything the player already holds. Skip days mean safe zones only, never punishment. (Ch. 3)
3. **Towns stay saved.** Six towns, six Lantern Seals, each earned by a library-study Bible Quest and a staged trivia battle vs the town champion, alongside a restoration meter filled by service quests. A saved town permanently transforms — color, lanterns, music, shops — and opens the road onward. Trials (Murkmire, Whisper Gorge, Hollowkeep) are survived, not saved. (Chs. 3, 7–16)

**Multiplayer** is exactly one thing: instanced 2–8 player Mission Trips within a single youth group, launched from boards in saved towns, communicating only through a quick-phrase/emote wheel. Missions earn group renown that feeds the Garden League. (Ch. 4)

**Scope.** One hub (Home Garden + Community Garden), six seal towns, three trial zones, one finale city, four Pokémon-style late-to-early shortcuts plus a ferry loop and endgame Ember flight, one combat system with 11 bestiary entries and 5 bosses, 4 launch co-op missions, and a full economy that shares the Backyard's existing wallets. Everything is specified for low-poly three.js r128 in a mobile WKWebView: ≤120 draw calls, ≤150 k triangles, 30 fps floor on an iPhone 8-class device. Audience 11–18, church youth groups, COPPA-safe; parents and pastors see everything.

**Build plan.** Part III derives three phases: **Phase 1** is a vertical slice — Home Garden annex, Meadow Town, Riverbend, Murkmire, tier-1 combat, lantern basics, and the Flood Rebuild co-op mission — proving the full loop (read → lantern → frontier → save a town → serve together). **Phase 2** builds the dark middle (Lantern Hollow through Hollowkeep) and the full mission suite. **Phase 3** ships the summit, the harbor, Everlight City, and the endgame.

**Non-negotiables.** Light is earned, never bought — no purchase touches brightness, verses, or seals. Scripture is quoted accurately with references or not quoted at all. Glimmer (counterfeit shine) is always distinguishable from glow (true light). Nothing a player owns is ever taken away.
...
- **Atlas anatomy.** Every world entry uses the same nine sections — 1 Fantasy & role, 2 Visual direction, 3 Layout, 4 Characters, 5 Quests, 6 Unique mechanic, 7 Secrets, 8 Audio, 9 Gating — and opens with a **Seam** callout stating the gating chain into and out of the zone.
...
| Verse Satchel | The verse loadout/collection; cards, families, mastery |
...
17. Phase 1 — the vertical slice
18. Phase 2 — the dark middle
19. Phase 3 — summit, harbor, source
...
|---|---|---|---|
| 1 | **Light is earned, never bought** | Frontier access, encounter strength, and endgame gates run through real reading and study. Gold buys tools, seeds, cosmetics — never brightness, verses, or seals. | Any IAP, gold sink, or shortcut that substitutes for reading. Any "watch ad / pay to relight lantern" mechanic. Verse Satchel purchases. |
...
resource node / quest marker)
  ...
  → immediate feedback: light spreads, restoration meter ticks, Standing points,
    gold/water/XP into the SAME wallets the Backyard uses
...
```
Open app → daily Bible plan (existing) → lantern lights / brightness tier set
  → Home Garden: water crops, harvest, sell (existing loop, 2–4 min)
...
|---|---|---|
| Home garden farming | Unchanged; becomes the hub's economic engine. Glowlands adds *demand* (service quests consume fruit, Toolworks gear needs materials) without touching supply mechanics. | No Glowlands feature may alter grow times, watering, or plot rules. |
| Water / XP from reading | Same earn events now also feed the Lantern. One reading session → water + XP + brightness, one action, three payouts. | Reading is never double-charged: no separate "Glowlands reading" requirement. |
| Gold / Berry Market | Single shared wallet. Town shops in saved towns price in the same gold. | No second currency for the overworld. Renown (group) and Standing (per-town) are reputations, not spendable. |
| Ember | Travels with the player from minute one; comic relief in towns, silent in Gloom territory (tone-zone rule), endgame mount. | Ember never fights and never speaks lies or verses on the player's behalf. |
| Eli | Gives the prologue call-to-adventure at the garden gate and later connects to the orchard-tunnel shears (via his sister Brama, Ch. 10). | Eli stays home; he is the reason the Backyard still feels tended. |
...
Combat in Glowlands is an argument you win, not a body you break. Gloomlings attack with written lies; the player answers with Scripture from the Verse Satchel. The correct truth produces a Lightburst and the darkness flees (James 4:7 — "Resist the devil, and he will flee from you"). The wrong answer never hurts the player's body — it lets fear close in. Nothing in this system deals damage, bleeds, or dies. Darkness always flees light.
...
4. **Answer.** Player selects a verse card. Correct answer = **Lightburst**: the verse text flares on screen for 1.2 s with its reference, the Gloomling recoils, dissolves upward into fireflies, and drops Ember-sparks (see combat economy) plus occasional verse fragments. Wrong verse = **fear closes in**: vignette snaps 15% tighter, the lie re-forms louder, and that card greys out for this encounter (no spam-guessing).
...
### 2.3 Verse Satchel — loadout & UI

- **Earning.** Verses drop from library study with town historians, Bible Quest milestones, and real Bible-plan reading in the app (each completed plan day grants one verse pull from that day's passage pool). No verse is purchasable — truth is learned, never bought.
- **The passage pool (content pipeline).** Every plan day ships with a curated pool of **≥3 combat-tagged verses**, hand-picked from that day's passage where viable. When the day's passage has no viable candidate (genealogies, census lists, itineraries), the pull draws from the plan's **authored fallback pool** — a per-plan reserve of ≥5 verses per family (30 minimum), written into the CMS at plan authoring time. Tagging is a writers'-room job, never automated: each tag assigns exactly one family, and each verse card carries its reference plus a verbatim excerpt of ≤12 words from the plan's translation (full text on long-press). A plan cannot publish with an empty day pool — the CMS blocks it. Net effect: every plan day in the app, in any book, yields a usable combat verse.
- **Loadout.** 6 family slots × up to 3 verse cards (max 18 equipped; collection unlimited). Editable at any lantern or in Home Garden; locked during encounters.
- **UI.** Radial wheel, bottom-center, thumb-reach. Outer ring = 6 family wedges; tapping a wedge fans out its 3 cards showing reference + first four words. Long-press a card to read in full (pauses the timer in single-player). One-handed portrait play is the design target.
- **Mastery.** Correct use of the same verse 5 / 15 / 40 times upgrades it Bronze/Silver/Gold: Gold cards Lightburst 30% larger and grant +10% Ember-sparks (tunable). Memorization through repetition, not grind gates.

### 2.4 Timing & difficulty model

Difficulty scales along two independent axes — **pressure** (time) and **precision** (answer exactness). Trials spike pressure only, per the tone rules; precision advances strictly in journey order.
...
**Co-op modifiers (Mission Trips).** Time dilation and read-in-full pause are disabled in instanced co-op — the world stays real-time for 2–8 players. To compensate, Mission lies are always family-precision regardless of tier, on a shared 20 s timer (tunable) visible to all players, and any teammate's correct answer clears the lie for the group (the Lightburst originates from the answerer). Wrong answers grey the card only for that player. In Missions, the no-soft-lock generator deals only lies counterable by the **union of all present players' equipped families**; if per-player grey-outs exhaust that union mid-encounter, timer expiry resolves via the Fade path as in single-player — the group wakes together at the mission's last checkpoint lantern. The emote wheel auto-suggests the family icon a teammate just identified — coordination without free chat.
...
### 2.7 Armor of God — gear effects

Story-milestone gear (Eph. 6:10-17), never dropped, never sold. One legible effect each. **This table is the single spec for gear effects; atlas entries state the grant and point here.**

| Piece | Milestone | Effect (tunable) |
|---|---|---|
| Belt of Truth | Seal 1 (Meadow Town) | Lie's key word underlines in family color — permanent taxonomy hint |
| Shoes of Peace | Seal 2 (Riverbend) | Satchel opens 40% faster; full move speed while open |
| Breastplate of Righteousness | Murkmire trial | Wrong answers tighten vignette 10% instead of 15% |
| Shield of Faith | Seal 3 (Lantern Hollow) | Once per encounter, tap-block a lie before it lands (extinguishes "flaming arrows"; skips it) |
| Helmet of Salvation | Seal 5 (Starcrest) | Vignette decays 2× faster post-encounter; immune to Dreadroot pre-tighten |
| Sword of the Spirit | Seal 6 (Brightharbor) | Gold-mastery Lightbursts chain to one identical-family lie within 6 m |

### 2.8 Failure states — fear, never death

No HP, no death. The vignette is the only health bar. If fear fully closes (repeated misses / expiries), the player **Fades**: soft black, a heartbeat, then Ember's snort — the player wakes at the nearest lit lantern or safe zone, items and progress intact, solved lies staying solved. Cost is position plus a 10 s Ember pep line. Missed lies enter the Echo list, resurfacing via Echo Shades and library review prompts — failure literally becomes curriculum.
...
- **Read-aloud:** every lie and verse card has recorded VO (reuses the verse-audio narration pipeline); lies whispered, verses spoken warm.
...
The Glowlands runs on four resources with hard-separated purposes: **gold** (trade — earned in-world, spent in-world), **XP** (the whole-journey measure that gates level-locked gear), **water** (the garden's founding growth resource, earned by real Bible reading), and **lantern brightness** (bound exclusively to real reading in the app; no quest, purchase, or grind can raise it). Every system below routes back to one of these four so the player always knows which loop they are advancing.
...
Levels gate **gear equip requirements only** — never map access (that is the Lantern's job) and never verses (that is study's job). Level-up grants: +1 Verse Satchel slot every 5 levels (base 6, max 12), a gold purse bump, and a light-burst cosmetic moment at the player's feet (ring of grass-glow decals, 1.2 s, ≤40 sprites for phone budget).
...
|---|---|---|---|
| **Source** | Fruit sales (Berry Market) | 8–120 per fruit by seed tier | Primary income, all game |
...
Water's loop is deliberately untouched by Glowlands: earned by real reading, spent watering home-garden crops, exactly as today. The expansion adds one optional sink and no new sources — **restored town gardens**. Each saved town exposes a small communal plot (4–6 beds, tunable) that any Guardian-standing player may water; watered beds bloom for every visitor for 24 h and tick +1 Standing per day (capped 1/day/town). This keeps water scarce enough to matter without inflating it, and gives veterans a reason to carry a full can into the world. No world activity ever charges water as a fee — it is a gift resource, spent only on making things grow.
...
**Onboarding rule (closes the day-1 gate):** Meadow Town's core district is always accessible pre-seal-1 regardless of tier — it is the world's sole gateway and can never be dark to a newcomer. The prologue's first beat *is* the first plan day: completing it lights the lantern to Flame on the spot, so every player exits the prologue with a lit lantern and the Gloom already driven from the square.
...
|---|---|---|---|
| Armor of God (6 story slots) | Belt of Truth, Shoes of Peace, Breastplate of Righteousness, Shield of Faith, Helmet of Salvation, Sword of the Spirit | Story milestones only, one per major arc | Mechanical, one legible effect each — single spec in the Armor of God table (Ch. 2.7) |
...
|---|---|---|
| Daily | Read plan day → lantern rises a tier → water/XP → tend garden → one red-bag question → optional 1 frontier push | 8–15 min |
...
Mission Trips are the only multiplayer play in the Glowlands: instanced, drop-in co-op service missions for 2–8 players from the same youth group, launched from mission boards in saved towns — the game's answer to the real-world church mission trip. Main-map exploration stays single-player; Mission Trips are where the group shows up.

### 4.1 Matchmaking and lobbies

No public matchmaking. The pool is always the player's own youth group (the Garden League roster), so every session is a known-safe, moderated cohort.
...
1. Player taps **Start a Trip** at a town board → creates a lobby; or **Join** on an open groupmate lobby (from any board or the Trip Tracker).
...
Campfire is unskippable by the host (players may individually leave). It is the social payoff: everyone's contribution is named, nobody's is ranked as "worst."
...
Scaling is **workload, not danger**. Missions never add tougher enemies at higher counts; they add more need.

| Dial | Rule (tunable) |
|---|---|
| Task volume | base × (0.6 + 0.4 × players⁰·⁹) — sublinear, so small groups aren't crushed |
| Timer | fixed regardless of count; pressure comes from volume |
| Multi-carry objects | objects tagged 2-person at ≥4 players (beam carry, stretcher); solo-carry fallback below 4 |
| Turn intensity | complication spawns scale 1 per 2 players |
| Duo comfort | at 2–3 players, one NPC helper (Eli or a townsfolk) auto-joins fetch tasks |
...
The river has breached the east levee; the Miller farmstead is knee-deep. Riverbend's signature mission and the vertical slice's co-op proof (Ch. 17).
...
A Gloom pocket has crept back into the mill district after dark; four townsfolk are stranded in it. The only combat-flavored launch mission, and the launch implementation of the **city-rescue archetype** — the mill district stands in for a city until the full urban version, **Glimmerton Blackout** (first post-launch mission), ships. Minimum is 2: the standard duo-comfort NPC helper covers the verse duet, so no floor above the global one is needed.
...
- **Torchbearer rule.** A groupmate below Flame may still join as a *Torchbearer*: the chapel lends them a fixed-radius lantern (no radius buffs, no Beacon/Shieldbearer roles), and they earn full mission rewards. The gate motivates reading; it never benches anyone on group night.
- **Roles.** *Beacon* (+20% radius), *Seeker* (rescue pings at 2× range), *Shieldbearer* (requires Shield of Faith story gear; blocks one wrong-verse fear pulse for the group per rescue).
...
- **Rewards.** Renown 150/110/70; 50 gold; 1 guaranteed new Verse Satchel verse; Gold adds the **Hollow Lamplighter's Hood**.
...
| Double-tap Ember button | Ember hop-to-shoulder / dismount (endgame flight uses the same button, context-sensitive; see camera: flight). |
...
The HUD speaks lantern, not menu. Persistent elements: lantern brightness meter (top left, drawn as the actual lantern), water/gold/XP pill (top right, collapses to icons after 3 s), context button (bottom right), Ember button (bottom left). Nothing else lives on screen while roaming.
...
| Verses earned | Server | Never shown as earned until server-acked — verses gate combat, so an unconfirmed verse must not enter the Satchel |
...
| Zone type | Direction |
|---|---|
| Saved towns / garden | Acoustic folk — guitar, hand percussion, whistling. Saving a town adds the melody stem; unsaved towns play the same bed hollowed out (no melody, detuned 15 cents). Players hear salvation. |
...
Multiplayer is quick-phrase/emote only (LOCKED); the phrase list ships server-side so it can be curated without an app release. No text entry exists anywhere in Glowlands — names come from the parent app's existing moderated profiles. Mission rosters are same-youth-group only, so every co-op session is already inside a supervised community. Parents and pastors can view play history, verses earned, and mission participation through the existing oversight surfaces; nothing in Glowlands is hidden from them (LOCKED). Report flow: long-press a player in a mission → "Tell a leader" → routes through the app's existing moderation queue. All screenshots/shares go through the parent app's controls; the game itself has no external share.
...
Eleven entries in journey order. Every entry opens with a **Seam** callout — the gating chain into and out of the zone — and uses the same nine numbered sections. The full chain, end to end:

```
Home Garden ─(Eastgate, opens in prologue)→ Meadow Town
  ─(East Gate: Seal 1 + restoration)→ Riverbend
  ─(Locks gate: Seal 2 + restoration; Flame lantern beyond)→ Murkmire
  ─(Willow Arch: ≤2 Burden, Mire-Mother outlasted)→ Lantern Hollow
  ─(East Palings: Seal 3 + restoration)→ Glimmerton
  ─(Gorge Gate: Seal 4 + restoration; Flame lantern beyond)→ Whisper Gorge
  ─(Hollowkeep Doors: one-way on first entry)→ Hollowkeep
  ─(Sun Gate: Dawnkey)→ Starcrest
  ─(North Saddle: Seal 5 + restoration)→ Brightharbor
  ─(ferry: downstream-only from arrival; two-way at Seal 6 + restoration)→ Meadow Town
...
> **Seam.** Home Garden → Meadow Town: the **Eastgate** arch opens in the prologue and never closes; the east road is a 20 s safe, Gloomless walk to Meadow Town's West Gate. The Community Garden's only gate is the south gate to Home Garden — it has no exit to the world and no quest ever routes through it. Post-finale, Everlight City's Wicket of the Morning drops back into the Home Garden. Nothing here is ever at risk or gated.
...
**7. Secrets.** (a) Watering the Scarecrow 7 days running grows a sunflower that drops 5 gold daily (tunable). (b) A journal page sits under the repaired Wayfarer's Table glass, written in glow-ink: at lower brightness tiers it renders as a faint animated shimmer across blank parchment (the breadcrumb that brightness matters here); when a Radiant player holds their lantern flame over the glass, the ink blooms legible over 0.6 s (tunable) and hints at the orchard tunnel. This is the one Hearthlight exception — the reveal keys off the player's carried lantern tier, not scene lighting, so the permanently bright garden never contradicts it. (c) Ember's shed-roof nap spot hides a scratched arrow pointing at Meadow Town's chapel — payoff lands with endgame flight.
...
**5. Quests.** Main beat: none — deliberately. Side quests: **"Raise the Arbor"** — the group's first collective goal: a shared water pool with a target of **10 × active members** (active = opened the game that League week), **capped at 200 water total** (tunable); any member may contribute any amount, over-contribution welcome, and the trellis's build state renders the pool percentage live. The quest **auto-completes when 60% of the target lands within a single League week**, so three enthusiastic kids in a 40-member group can finish it and no group stalls on absentees; the pool resets if the week ends short, with contributed water refunded. **"Naomi's Album"** — revisit three Trip Wall frames with Naomi for renown bonuses. Running joke: the Trip Wall auto-composes group photos and Ember photobombs every single one.
...
> **Seam.** In: the east road from Home Garden — always open, and the core district is accessible at any brightness (onboarding rule, Ch. 3.4). Out: the **East Gate** to Riverbend opens only when the town is saved (Seal 1 + restoration 100) — never by any other means. Later re-openings: orchard tunnel to Lantern Hollow (shears, post-seal-3), ferry departures to Brightharbor's loop (post-seal-6; downstream *arrivals* from day one of Brightharbor), Ember steeple landing (endgame), and the Pilgrim Stair to Everlight City rising above the town (six seals).
...
**5. Quests.** **Prologue (12–15 min, tunable):** Eli walks you to the West Gate at dawn → fog rolls over the east fields → first Gloomling whispers "You're too small to matter"; combat tutorial with the two starter verses (John 8:12 from Eli's card; Philippians 4:13 granted on the spot), guaranteed win → you find Bram frozen mid-field by "One mistake and you're done," but your satchel has no Grace card → Maribel's first study session earns 3 verses → free Bram with the right one → clear 4 Gloomlings, fog recedes. The prologue also lights the lantern to Flame (its first beat is the first plan day, Ch. 3.4), and **its combat begins the global miss-log** — every failed counter from here on is recorded for Echo Shades and Hollowkeep (Ch. 13 data contract).
...
- **Sim** — a kid who insists Ember could power the mill "if he'd just breathe on it right." Ember refuses with escalating theatrical dignity.
...
*Side quests.* **"Cobb's Log"** — recover the lockkeeper's waterlogged journal pages from three rooftops. **"Seeds Above Water"** — Rosie's cousin pays a flood premium for Glowberries grown back home: 12 gold each vs Berry Market's 8 (tunable, always ≥130% of Meadow Town price so the first cross-map farming loop pays). Running joke: every NPC has a different, mutually contradictory story about what Old Cobb did "in the Great Flood of aught-nine"; Cobb confirms all of them.

*Reward table (all tunable).*

| Quest | Water | XP | Gold | Other |
|---|---|---|---|---|
| Rope-walk / Locks / rooftop encounters | 2 each | 50 each | — | — |
| Library study (×3 sessions) | 4 each | 150 each | — | 1 verse each |
| Trivia battle win | 10 | 500 | 25 | Seal 2, Shoes of Peace |
| Sandbags (per bag ×12) | — | 10 | 2 | — |
| Prews rescue | 6 | 200 | 15 | — |
| Grain / Locks puzzle | 6 | 200 | 15 | — |
| Footbridge planks | 4 | 100 | 10 | — |
| Cobb's Log | 3 | 120 | 20 | — |
| Flood Rebuild mission (first clear) | 8 | 300 | 30 | Renown 120/90/60 by tier (Ch. 4.5; a Gold clear ≈ 40% of a solid Garden League week; repeat clears 50) |
...
**7. Secrets.** (a) **The Dry Chapel** — the half-sunk steeple hides an air-pocket room reachable only at Burden 0, containing a waterlogged Satchel page: every word is ruined except the reference, *Psalm 55:22*. Finding it unlocks a "Psalm 55:22" study assignment at the Lantern Hollow library; completing that study grants the verse. Discovery is the magic, the library is the source. (b) **Fenwick's manifest** — a journal page under the cart seat revealing he's carrying Glimmerton trade goods, foreshadowing the carnival's counterfeit economy. (c) **Clover's bell** — half-buried in Peddler's Bog; return it and Clover follows you to the Arch, and Ember, mortified, rides on your shoulder the whole way.
...
**9. Gating.** Murkmire is survived, not saved: no seal, no restoration; it never transforms, and the Gloom here always remains. Passing requires reaching the Willow Arch at ≤2 Burden (quest items exempt; final stump adjacent) with Fenwick's outcome resolved at any tier, then outlasting the Mire-Mother's three waves. Surviving the trial grants the **Breastplate of Righteousness** (Armor of God table, Ch. 2.7 — wrong answers tighten the vignette 10% instead of 15%). Unlocks: the road to Lantern Hollow, the Letters delivery hook, the Psalm 55:22 study assignment if the Dry Chapel was found, and the permanent **Traveler's Rule** — Burden Weight stays active in all later dark zones, making Murkmire the tutorial for every trial after it.
...
**7. Secrets.**
- **The Firefly Grove** — behind a curtain-fall spring east of the chapel; fireflies orbit a wild Glowberry bush; one-time Starberry cross seed.
...
**5. Quests.** Seal arc: (1) Pry open the library's boarded windows — a 90-second (tunable) light-the-lamps intro. (2) Three library study sessions on counterfeit light, each adding its verses to the Verse Satchel: Matthew 6:22-23 (the eye is the lamp), 2 Corinthians 11:14 ("Satan himself masquerades as an angel of light"), John 8:12 (the light of life). (3) The Mirror Maze seal battle vs Marlow — full spec below. Beat him and his hat falls off; a Gloomling has been whispering in his ear the whole time. It flees his Lightburst; Marlow, freed, awards **Seal 4**.
...
**4. Characters.**
- **Old Wickett** — lamplighter hermit, midpoint. Deaf in one ear and glad of it ("Can't lie to half of me"). Function: trial guide, waystone teacher, comic relief.
- **Harrow** — peddler camped at Gatelight Landing for "three seasons, briefly." Sells oil, wicks, satchel refills. Function: vendor, running joke.
...
> **Seam.** In: the **Gorge Door** portcullis at the end of Whisper Gorge — requires surviving the gorge with a lit lantern (any tier); it seals behind the player, no backtrack until escape. Out: the **Sun Gate**, opened only by the Dawnkey, releasing onto the Starcrest road. The Dawnkey also permanently unlocks the **wicket-gate backpath to Glimmerton**, folding the map's scariest zone against its brightest. Survived, not saved.
...
**3. Layout.** Linear with one loop. Entry gate: the Gorge Door, a portcullis at the end of Whisper Gorge that seals behind the player (no backtrack until escape). Order: Gatehouse (echo tutorial, below) → Gallery of Lanterns (hub; prisoners held here) → Archive of Echoes (library analog — study cells where lie/verse pairs are researched) → the Vault Stair (three verse-locks: each lock is a lie carved in stone with three satchel verses offered; a wrong verse snuffs the ember for 5 s, tunable, and reshuffles the options; a verse used on any lock is consumed for the remainder of the Stair, forcing deep cuts instead of John 8:12 three times) → Keeper's Rotunda (duel arena, ringed by hung lanterns instead of a crowd) → Sun Gate (exit; opens only to the Dawnkey, releases onto the Starcrest road). The wicket gate — a person-sized door in the Gallery's south wall — is findable early but locked; it opens outward to a Glimmerton alley once the Dawnkey is held. No market, mission board, or chapel — but see secrets.
...
> **Seam.** In: the **Gorge Stair** in the south cliff — requires only the Dawnkey escape from Hollowkeep; Starcrest itself is a full safe zone, enterable and walkable at any brightness (fear-zone lamb behavior, Flame+, is the only brightness-sensitive system inside it). Out: the **North Saddle** gate behind the chapel, sealed until the town is saved (Seal 5 + restoration), opening the descent road to Brightharbor. Shortcut: the one-way **Ledge drop** to Glimmerton — the jump prompt arms only when Starcrest is saved.
...
**3. Layout.** Map footprint ~180×220 m; entry gate to North Saddle exit ≈ 90 s at walk speed along the main loop, ≈ 45 s to the chapel. Walkable loop, entry to exit: **Entry gate** — the Gorge Stair, a switchback cut in the south cliff (door from Whisper Gorge/Hollowkeep exit). It opens onto **Cloudrim Walk** (~60 m overlook path along the cloud floor) → **Bellfield** (three pasture terraces of ~40 m each, needy-NPC shepherd camps, side quests) → **Town Terrace** (market row: wool-and-broth stalls, a Rosie's Seeds satellite cart, **mission board** by the well) → **The Star Library** (observatory-library hybrid, historian, dome with a low-poly orrery) → **Chapel Green** (chapel + steeple; the quest arena is the open green in front, crowd seating on the terrace steps) → **Exit gate**: the North Saddle gate behind the chapel, sealed until the town is saved, opening the descent road toward Brightharbor. **The Ledge** (one-way drop to Glimmerton) sits west of Cloudrim Walk, deliberately off the main loop.
...
**8. Audio.** Music: concertina and low strings over a slow 6/8 sway; restoration adds hammered dulcimer and gull-cry ornaments. Signature sounds: the fog's sub-bass *hush* that ducks all other audio near the waterline; the Tide Clock's brass *clunk* at each tide turn, audible town-wide. The Undertow boss arc (Ch. 2.6) plays over this bed — the drowned-lantern stingers are its percussion.
...
- **Armor of God modifiers (Vigil-specific overrides of the Ch. 2.7 table):** Belt of Truth greys out one wrong satchel option on compound lies; Shield of Faith absorbs the first wrong answer per wave (no wedge darkens); Helmet of Salvation +5 s per timer; Shoes of Peace +50% Kindling reticle speed; Breastplate of Righteousness retreats the Hush one ring on any 3-correct streak; Sword of the Spirit makes correct answers ignite 2 seats at the chosen origin instead of 1.
...
**Sending Quay.** Post-game Mission Trips (2–8 players, same youth group, quick-phrase wheel only) escalate: Gloom-remnant cleanups in Whisper Gorge, rebuild contracts in Riverbend and Brightharbor, VBS hosting in Glimmerton. Lantern Row consumables (flares, loaves, oil) are balanced for these. Renown from Quay missions feeds the Garden League weekly.
...
The phasing rule: **every phase ends with something a real youth group can play end-to-end.** Phase 1 proves the whole thesis on the smallest possible map; Phases 2 and 3 extend the proven loop rather than betting on new ones. Content within a phase is ordered so systems land before the zones that consume them, and so the Hollowkeep data dependency (the global miss-log, Ch. 13.6) is satisfied from the first shipped encounter.

## 17. Phase 1 — the vertical slice

**Scope.** Home Garden annex + east road + Meadow Town (full) + Riverbend (full) + Murkmire (full trial) + Truth & Light at tier-1 precision + lantern basics + one co-op mission (Flood Rebuild) + economy, save/sync, and oversight surfaces. This is canon's Phase 1 slice — "Meadow Town + Riverbend + Murkmire + combat + 1 mission + lantern basics" — made concrete.

**Why this slice proves the game.** It exercises every pillar once: reading lights the lantern and opens Murkmire (pillar 1); the prologue through the Willow Arch runs the full non-violent combat ramp from Whisperling to the Mire-Mother (pillar 2); a lapsed player still has two saved towns and a garden (pillar 3); Meadow Town's save transformation is the retention promise made visible (pillar 4); and three zone biomes + one instanced mission stress the entire perf budget (pillar 5). It also contains the game's first trial, first boss (the First Shadow), first champion trivia battle, first two Armor pieces plus the Breastplate, and the group's first renown — nothing in Phases 2–3 introduces a *kind* of thing absent here except shortcuts, minigame parlors, and endgame systems.

**Ordered feature list (build order; each row assumes the rows above it).**

| # | Feature | Contents / notes |
|---|---|---|
| 1 | Engine frame | Zone streaming via gate transitions (≤4 s LTE), camera rig (Roam/Close/Encounter states), touch controls (tap-move, floating stick, contextual hop), movement feel + juice baseline (squash/stretch, footstep puffs, lantern pendulum, interaction bloop, haptics bridge). Perf harness asserting ≤120 draw calls / ≤150 k tris / 30 fps floor on the target device from week one. (Ch. 5) |
| 2 | Home Garden annex | Eastgate arch, Lantern Post, damaged Wayfarer's Table (six empty sockets), Satchel Hook, south-path retirement + bramble, Hearthlight rule, Ember/Eli/Pip prologue staging. Community Garden untouched except data hooks for later. (Ch. 6) |
...
| 4 | Verse pipeline + Satchel | CMS plan-day pool authoring (≥3 tagged verses/day) + per-plan fallback pools (30 min) + publish blocker; verse pull on plan-day completion, server-acked earns; Satchel radial UI, 6 families × 3 cards, long-press full text, mastery counters. **The global miss-log ships here** (`lie_id, counter_verse_ref, zone, timestamp`) — Hollowkeep's Phase 2 echo system reads it retroactively. (Chs. 2.3, 13.6) |
| 5 | Truth & Light core | Encounter flow (aggro → lie → satchel → Lightburst / fear-closes-in → resolve), tier-1 family precision, vignette system, Fade path, no-soft-lock generator (server), Ember-sparks wallet + steeple/garden spark store (small launch catalog). Bestiary: Whisperling, Murmur Pack (Worry-wisp skin), Heavyback. Boss framework + the First Shadow. Calm Mode + reduced-flash + read-aloud hooks. (Ch. 2) |
| 6 | Meadow Town | Full zone: prologue (12–15 min, lights lantern to Flame, starts miss-log), library study, trivia-battle template, Seal 1 + Belt of Truth, restoration meter + shops-as-service-organs, Gloom-stain creep, save transformation, Standing track, secrets, East Gate unlock. Seal socket + fruit-multiplier (+5%/seal) live. (Ch. 7) |
| 7 | Riverbend | Full zone: Worry-wisp encounters, Flow Routing (3 solo puzzles + reset levers), seal arc vs Tomas (first timed trivia), Seal 2 + Shoes of Peace, restoration weights, save transformation + tide-out, Locks gate to Murkmire. (Ch. 8) |
| 8 | Murkmire | Burden Weight (+Traveler's Rule flag), Last Dry Table storage, Sinking Causeway, Cart Rescue, prayer stumps, Willow Arch / Mire-Mother waves, Breastplate of Righteousness, Flame entry gate (the game's first brightness gate). (Ch. 9) |
| 9 | Mission Trips core | Relay-backed instancing (10 Hz transforms, server-authoritative objectives), board + lobby + push flow, quick-phrase wheel, five-beat spine, drop-in/reconnect rules, workload scaling, **Flood Rebuild** (with the co-op two-crank Flow Routing step). Boards live in saved Meadow Town + Riverbend; VBS Host is Phase 2, so the visiting-slot rotation ships pointing both boards at Flood Rebuild variants. Renown: lifetime + weekly tracks, Garden League weekly scoring hook (50/50 start), renown-ladder tier 1 (Trip Tracker). (Ch. 4) |
| 10 | Economy pass | XP curve + level-ups (satchel slots), gold sources/sinks live (donation caps, wayfarer kits, offering box → renown), water untouched, week-3 ledger and ten-week XP ledger re-run against real tuning data. (Ch. 3) |
| 11 | Persistence + trust | Save/sync split (local vs server), optimistic-with-visible-rollback currency, checkpoint resume rules, offline saved-town play, day-night LUT + Riverbend rain. (Ch. 5.5–5.6) |
| 12 | Safety + oversight | Parent/pastor visibility (play history, verses earned — counts not lie text, mission participation), report flow, Gentle mode, accessibility baseline (text scale, reduce-motion, colorblind double-coding), analytics events for the funnel (reading → gate → seal → mission). (Chs. 5.9–5.10, 13.6) |

**Exit criteria.** A new player reaches the Willow Arch from a cold start with no designer hand-holding; a lapsed player at Spark still has the garden and two saved towns with zero locked-out owned content; a 4-player Flood Rebuild holds 30 fps on an iPhone 8-class device over the live relay; the reading → lantern → Murkmire gate loop is verified against production reading data; both economy ledgers land within their bands.
...
**Zones.** Lantern Hollow (Lamplighting, brightness tiers surfaced diegetically, Seal 3 + Shield of Faith, orchard-tunnel shortcut) → Glimmerton (neon `uLanternRadius` shader, Glimmer Games + Tickets, Seal 4 + the Big Sell, Glimmer King) → Whisper Gorge (radius-as-resource, waystones, the Choir, Dawnlit Wick) → Hollowkeep (lantern suppression, Echoes consuming the Phase-1 miss-log, Vault Stair, Keeper duel, Dawnkey + wicket-gate shortcut).
...
- Beacon/Radiant gates go live (unsaved-town Gloom districts; frontier flags staged for Phase 3).
- Missions: VBS Host, Midnight Rescue (dark-flag gate + Torchbearer rule), Kickabout (playable via visiting-slot rotation ahead of its Brightharbor home board); full board rotation; renown ladder tiers 2–4; Community Garden group content (Renown Arbor, Trip Wall, Raise the Arbor, Naomi).
...
**Zones.** Starcrest (Crook Call, Cald buzzer battle, Seal 5 + Helmet of Salvation, Cragmother, Ledge drop) → Brightharbor (Tide Clock, fog-lane row-out, Sela battle, Seal 6 + Sword of the Spirit, Undertow, two-way ferry loop) → Everlight City (Six-Seal Gate, Lantern Row gift economy, Selah's 18-prompt review, Kindling, the Vigil, commissioning).
...
**Exit criteria.** All six seals + Vigil completable by a daily reader in ~10 weeks at level ~30 (Ch. 3.1 ledger holds); the ferry victory lap renders every saved town lit; a Vigil-complete account can fly, host Sending Quay trips, and start NJ+; one full seasonal surge cycle rehearsed on staging before the first live season.
